...
test("MessageList shows empty state when no messages", () => {
  ...
  expect(
    screen.getByText("Start creating with AI")
  ).toBeDefined();
  expect(
    screen.getByText("Generate beautiful React components with conversation. I can help you build buttons, forms, cards, and entire interfaces.")
  ).toBeDefined();
});
...
test("MessageList handles step-start parts", () => {
  ...
  // Check that a separator exists (div with gradient)
  const container = screen.getByText("Step 1 content").closest(".rounded-2xl");
  expect(container?.querySelector("div.h-px")).toBeDefined();
});
...
test("MessageList applies correct styling for user vs assistant messages", () => {
  ...
  expect(userMessage?.className).toContain("from-blue-600");
